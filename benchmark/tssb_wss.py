...
def evaluate_window_size(exp_path, n_jobs, verbose):
    ...
    for candidate_name, algorithm in algorithms:
        ...
        if candidate_name == "Human":
            window_sizes = df_tssb.window_size.to_numpy()
        else:
            window_sizes = dp.map(algorithm, df_tssb.time_series, expand_args=False, ret_type=np.array, n_jobs=n_jobs, verbose=verbose)
