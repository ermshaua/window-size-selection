...
def plot_grid_motiflets(
    ...
    if (show_elbows):
        ...
        ax_title.set_title('(c) Shape of Top Motif Sets by Method', pad=50)
    ...
    for i, motiflet in enumerate(motiflets):
        if motiflet is not None:
            ...
            for aa, pos in enumerate(motiflet):
                if pos + motif_length < len(data_raw) and pos > 0:
                    df[str(aa)] = zscore(data_raw[pos:pos + motif_length])
                    ...
                    rect = Rectangle(
                        ...
                        facecolor=sns.color_palette()[
                            (len(ground_truth) + i % grid_dim) % len(sns.color_palette())],
                        # hatch=hatches[i],
                        alpha=0.7
                    )
            ...
            if method_name is not None:
                ...
                y_labels.append(method_names[i].replace("\n", " "))

            dists = ""
            plot_dists = dist is not None
            if plot_dists:
                dist = np.array(dist)
                dist[dist == float("inf")] = 0
                dists = str(int(dist[elbow_points[i]]))

            label = "k=" + str(len(motiflet))
            if plot_dists:
                label = label + ",\nd=" + dists
            ...
            _ = sns.lineplot(ax=ax_motiflet, data=df_melt, x="time", y="value", ci=99,
                             color=sns.color_palette()[
                                 (len(ground_truth) + i % grid_dim) % len(
                                     sns.color_palette())],
                             label=label
                             )
    ...
    if method_names is not None and save_fig:
        if idx is None:
            plt.savefig("../../experiments/motifs/images/" + name.replace(" ", "-") + "_grid.pdf",
                        bbox_inches='tight')
        else:
            plt.savefig(
                "../../experiments/motifs/images/" + name.replace(" ", "-") + "_grid_" + str(idx) + ".pdf",
                bbox_inches='tight')

    if show_elbows and save_fig:
        plt.savefig("../../experiments/motifs/images/" + name.replace(" ", "-") + "_elbows.pdf",
                    bbox_inches='tight')
