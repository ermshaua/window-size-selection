...
def read_ground_truth(dataset, path="/"):
    file = path+"datasets/motifs/" + dataset.split(".")[0]+"_gt.csv"
    if (exists(file)):
        print(file)
        series = pd.read_csv(
            file, index_col=0,
...
def read_dataset_with_index(dataset, path="/", sampling_factor=10000):
    full_path = path+ 'datasets/motifs/' + dataset
    ...
    gt = read_ground_truth(dataset, path)
    if gt is not None:
        return data, gt
    else:
        return data
...
def read_dataset(dataset, path="/", sampling_factor=10000):
    full_path = path+'datasets/motifs/' + dataset
...
def find_elbow_points(dists):
    ...
    # add at least one motif: the pair motif
