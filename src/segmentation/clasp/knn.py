...
def _iterative_knn(time_series, window_size, k_neighbours, tcs, dot_first):
    ...
    if tcs is None: tcs = [(0, time_series.shape[0])]
...
def iterative_knn(time_series, window_size, k_neighbours, tcs=None, verbose=0):
    dot_first = _sliding_dot(time_series[:window_size], time_series)
    return _iterative_knn(time_series, window_size, k_neighbours, tcs, dot_first)